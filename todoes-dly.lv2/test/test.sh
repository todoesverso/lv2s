...
NAME_IN=test
NAME_OUT=test_out
URI_MONO=https://github.com/todoesverso/lv2s/todoes-dly#mono
...
sox -V -r 48000 -n -b 16 ${FILE_IN} synth 6 saw 500 vol -3dB
sox -V ${FILE_IN} -n stats
# Process
LV2_PATH=$(pwd)/plugin ${LV2FILE} --list 
LV2_PATH=$(pwd)/plugin ${LV2FILE} -n ${URI_MONO}
LV2_PATH=$(pwd)/plugin ${LV2FILE} -i ${FILE_IN} -o ${FILE_OUT} -c1:in -pdelay:10 ${URI_MONO}
sox ${FILE_IN} -n stats
